...
import org.springframework.pulsar.core.RestartableSingletonFactoryBase.State;
import org.springframework.test.util.ReflectionTestUtils;

/**
 * Tests for {@link RestartableSingletonFactoryBase}.
 */
public class RestartableSingletonFactoryBaseTests {
	...
	void startCreatesInstanceAndSetsStateToStarted(State initialFactoryState) {
		var fooFactory = spy(new RestartableSingletonFooFactory());
	...
	void stopDestroysInstanceAndSetsStateToStopped(State initialFactoryState) {
		var fooFactory = spy(new RestartableSingletonFooFactory());
		...
		inOrder.verify(fooFactory).destroyInstance(eq(foo));
	...
	void stopDoesNothingWhenStateIsInvalid(State initialFactoryState) {
		...
		verify(fooFactory, never()).destroyInstance(any(Foo.class));
	}
	...
	void createInstanceNotCalledWhenInstanceSetInConstructor() throws Exception {
		...
		var fooFactory = spy(new RestartableSingletonFooFactory(foo));
		fooFactory.afterPropertiesSet();
		fooFactory.start();
		verify(fooFactory, never()).createInstance();
	...
	void setFactoryState(RestartableSingletonFactoryBase<?> factory, State factoryState) {
		ReflectionTestUtils.setField(factory, "state", new AtomicReference<>(factoryState));
		verifyFactoryState(factory, factoryState);
	}

	void verifyFactoryState(RestartableSingletonFactoryBase<?> factory, State expectedState) {
		assertThat(factory).extracting("state").satisfies((state) -> {
			assertThat(state.toString()).contains(expectedState.name());
		});
	}

	static class RestartableSingletonFooFactory extends RestartableSingletonFactoryBase<Foo> {

		RestartableSingletonFooFactory() {
			super();
		}

		RestartableSingletonFooFactory(Foo instance) {
			super(instance);
		}
		...
		@Override
		protected void destroyInstance(Foo instance) {
